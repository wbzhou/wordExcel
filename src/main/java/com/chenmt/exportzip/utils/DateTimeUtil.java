...
public class DateTimeUtil {
	...
	public static Date parseTime(String timeString) {
		Date time = null;
		try {
			SimpleDateFormat tmpF = new SimpleDateFormat(HHmm);
			time=tmpF.parse(timeString);
	...
	public static long addSecNow(int secs) {
		return new Date().getTime()+secs*1000;
	}
	...
	public static Date getTicketTime(String strDate, String toDay) {
		if (StringUtils.isEmpty(strDate) || StringUtils.equals(strDate, "false")) {
			return null;
		}
	...
	/**
	 * @author:wangying
	 * 比较两个时间差，返回 XX小时XX分 date2-date1
	 * 需要将date2 timezong2转换成timezone1
	 * @param date1
	 * @param timezone1
	 * @param date2
	 * @param timezone2
	 ...
	 * 2018年8月14日
	 */
	public static String getDateDiffHourWithTimeZone(Date date1, String timezone1, Date date2, String timezone2){
		Date changeTime = changeTime(date2, TimeZone.getTimeZone(timezone2), TimeZone.getTimeZone(timezone1));
		int dateDiffMinute = getDateDiffMinute(changeTime, date1);
		String time = "";
		int hour = dateDiffMinute / 60;
		if(hour > 0){
			time += hour + "小时";
		}
		int minute = dateDiffMinute%60;
		if(minute > 0){
			time += minute+"分";
		}
		return time;
	 ...
	 * 2018年8月14日
	 */
	public static int getDateDiffMinuWithTimeZone(Date date1, String timezone1, Date date2, String timezone2){
		Date changeTime = changeTime(date2, TimeZone.getTimeZone(timezone2), TimeZone.getTimeZone(timezone1));
		int dateDiffMinute = getDateDiffMinute(changeTime, date1);
	...
	public static Date parseDate(String date, String format) {
		...
		try {
			...
			logger.error("解析时间错误,date:{}, format:{}", date, format, e);
		}
		return null;
	}
}
